...
// The render pass — Swift analogue of src/render/Renderer.ts. Paints the whole
// scene from GameModel state each frame in the same three transform regimes:
...
//
// The renderer is a per-frame value: it reads the model and PRODUCES the
// tappable-rect registry, returning it so the caller can commit it in one
// write. Drawing never mutates the model mid-frame (the web Renderer keeps the
// same separation — the View owns the model, the Renderer only paints).
struct GameRenderer {
  let model: GameModel

  /// Built up over a single draw pass and returned to the caller, which
  /// commits it to `model.buttons` in one assignment (true double-buffer:
  /// hit-tests read last frame's fully-formed registry, never a partial one).
  private var buttons: [ButtonID: CGRect] = [:]

  // Explicit init: the synthesized memberwise initializer inherits `buttons`'s
  // `private` access, which would hide it from ContentView.
  init(model: GameModel) { self.model = model }

  // ───────────────────────────────────────────── frame

  mutating func render(into ctx: inout GraphicsContext, size: CGSize) -> [ButtonID: CGRect] {
  ...
  private mutating func renderTitle(_ ctx: inout GraphicsContext, w: CGFloat, h: CGFloat) {
    ...
    if !model.explainerOpen {
      ...
      buttons[.begin] = begin
      buttons[.explainer] = drawExplainerLink(&ctx, w: w)
    } else {
      // Modal: BEGIN is not registered underneath, the card owns all input.
      buttons[.dismissExplainer] = drawExplainerCard(&ctx, w: w, h: h)
    }
  }

  private mutating func renderSetup(_ ctx: inout GraphicsContext, w: CGFloat, h: CGFloat, time: Double) {
    let layout = model.layout
    ...
    buttons[.massMinus] = minus
    buttons[.massPlus] = plus
  ...
  private mutating func renderResolved(_ ctx: inout GraphicsContext, w: CGFloat, h: CGFloat, time: Double) {
  ...
  private mutating func drawCornerControls(_ ctx: inout GraphicsContext, w: CGFloat) {
