...
- `npm test` — run all Vitest tests once (25 tests).
...
Before declaring a UI change done: `npm run dev` and actually play through both setup phases plus a full simulation. The tests cover physics + outcome + fit + starfield invariants; they do **not** exercise the canvas, controls, cursor logic, resize wiring, or `screenToLogical` — only a real browser does.

Note on Node: CI runs Node 22; local dev has been run on Node 20. There is no `engines` field pinning either, so the skew is unguarded — prefer Node 22 to match CI.

## Architecture (one-paragraph mental model)

`src/main.ts` boots a `Game` (`src/game/Game.ts`) which owns the state machine (`src/game/states.ts`), a `Renderer` (`src/render/Renderer.ts`), and — once a round starts — a `Simulation` (`src/physics/Simulation.ts`). The Game drives `requestAnimationFrame`; each frame it advances the `Simulation` (PEFRL 4th-order symplectic integration over exactly two `Body` instances under Plummer-softened Newtonian gravity, drained from a fixed-step accumulator), asks `outcomes.ts` to classify the current state (Playing / WIN / LOSE_*), and asks the `Renderer` to paint. The game renders in a **fixed 1280×800 design space** that the Renderer maps into the live viewport with a uniform contain-fit (`computeFit`), letterboxed and DPR-sharp; `Renderer.screenToLogical` inverts that fit so pointer hit-tests line up. The `src/ui/` controls (`PositionControl`, `MassControl`, `ArrowControl`, plus the `input.ts` geometry helpers) are **passive mutators** of a `BodySpec`, active only during the Setup states — the Game owns all pointer/wheel/keyboard wiring and polls the controls' getters; the controls emit no events.
...
- All colors come from `src/theme.ts`. Never hard-code a hex outside that file. (The one exception in the tree is the iframe-refusal fallback HTML in `main.ts`, which hard-codes three hexes because it runs before the module graph is trusted; don't add more.)
